...
import { ShieldAlert, Users, FileWarning, LogOut, Ban, CheckCircle2, RefreshCw, Moon, Sun } from 'lucide-react'
      ...
      <div style={{ minHeight: '100vh', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 24, padding: '2.5rem', maxWidth: 420, width: '90%', backdropFilter: 'blur(24px)', boxShadow: '0 24px 64px rgba(92,49,242,0.12)', textAlign: 'center' }}>
          ...
          <button
            ...
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '0.875rem 1.5rem', borderRadius: 14, background: 'linear-gradient(135deg,#5C31F2,#7C3AED)', color: '#fff', fontWeight: 700, fontSize: '0.95rem', border: 'none', cursor: 'pointer', boxShadow: '0 4px 16px rgba(92,49,242,0.4)' }}
          >
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: '2rem' }}>
          {[
            ...
            { label: 'High Risk (2+ reports)', value: highRiskCount, color: '#FF844B' },
            { label: 'Total Reports', value: reports.length, color: '#7C3AED' },
          ].map((stat) => (
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem' }}>
          {(['users', 'reports'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.6rem 1.25rem', borderRadius: 12, background: tab === t ? 'linear-gradient(135deg,#5C31F2,#7C3AED)' : inputBg, border: `1px solid ${tab === t ? '#5C31F2' : border}`, color: tab === t ? '#fff' : muted, fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer' }}>
              {t === 'users' ? <Users size={14} /> : <FileWarning size={14} />}
              {t === 'users' ? `Users (${users.length})` : `Reports (${reports.length})`}
            </button>
          ))}
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${border}` }}>
                    {['Alias', 'Status', 'Reports', 'Tags', 'Joined', 'Actions'].map((h) => (
                ...
                <tbody>
                  {users.map((u, i) => (
                    <tr key={u.uid} style={{ borderBottom: i < users.length - 1 ? `1px solid ${border}` : 'none', background: u.status === 'suspended' ? (dark ? 'rgba(255,83,83,0.04)' : 'rgba(255,83,83,0.02)') : 'transparent' }}>
                      <td style={{ padding: '0.875rem 1.25rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${u.uid}&size=32`} alt="" style={{ width: 32, height: 32, borderRadius: '50%', background: '#E3DCF8' }} />
                          <div>
                            <p style={{ fontWeight: 700, color: text, fontSize: '0.875rem' }}>{u.username}</p>
                            <p style={{ fontSize: '0.72rem', color: muted, fontFamily: 'monospace' }}>{u.uid.slice(0, 10)}…</p>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: '0.875rem 1.25rem' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '0.25rem 0.75rem', borderRadius: 999, fontSize: '0.75rem', fontWeight: 700, background: u.status === 'active' ? 'rgba(34,197,94,0.1)' : 'rgba(255,83,83,0.1)', color: u.status === 'active' ? '#22C55E' : '#FF5353', border: `1px solid ${u.status === 'active' ? 'rgba(34,197,94,0.25)' : 'rgba(255,83,83,0.25)'}` }}>
                      ...
                      <td style={{ padding: '0.875rem 1.25rem', maxWidth: 200 }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          ...
                          {u.tags.length > 3 && <span style={{ fontSize: '0.68rem', color: muted }}>+{u.tags.length - 3}</span>}
                        </div>
                      </td>
                      <td style={{ padding: '0.875rem 1.25rem' }}>
                        <span style={{ fontSize: '0.8rem', color: muted }}>{formatDate(u.createdAt)}</span>
                      </td>
                      <td style={{ padding: '0.875rem 1.25rem' }}>
                        {u.status === 'active' ? (
                          <button onClick={() => handleBan(u.uid)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0.4rem 0.875rem', borderRadius: 8, background: 'rgba(255,83,83,0.1)', border: '1px solid rgba(255,83,83,0.25)', color: '#FF5353', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}>
                            <Ban size={11} /> Ban
                          </button>
                        ) : (
                          <button onClick={() => handleUnban(u.uid)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0.4rem 0.875rem', borderRadius: 8, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', color: '#22C55E', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}>
                  ...
                  {users.length === 0 && (
                    <tr><td colSpan={6} style={{ padding: '3rem', textAlign: 'center', color: muted }}>No users found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
